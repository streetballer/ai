...
from src.modules.teams.logic.get_standings_logic import get_standings

router = APIRouter()


@router.get("/teams/standings", response_model=ResponseModel)
def get_standings_route(player_id: str = Depends(get_current_player_id)) -> ResponseModel:
    result = get_standings(player_id)
    if result is None:
