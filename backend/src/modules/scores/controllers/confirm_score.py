...
from src.modules.scores.logic.confirm_score_logic import confirm_score

router = APIRouter()


@router.post("/scores/{score_id}/confirm", response_model=ResponseModel)
def confirm_score_route(
    score_id: str,
    player_id: str = Depends(get_current_player_id),
) -> ResponseModel:
    result = confirm_score(score_id, player_id)
    if result is None:
