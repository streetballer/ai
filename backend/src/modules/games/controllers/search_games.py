...
from src.modules.games.logic.search_games_logic import search_games_by_court, search_games_by_location

router = APIRouter()


@router.get("/games", response_model=ResponseModel)
def search_games(
    ...
    if has_court:
        result = search_games_by_court(court_id)
        if result is None:
            raise HTTPException(status_code=404, detail="Court not found")
    else:
        result = search_games_by_location(lon, lat)
